function TasksTask(name, multiple, useLocalCache, inputName) {
	this.name = name;
...
TasksTask.prototype.select = function(e)
{
	...
	if (!this.multiple)
	{
		...
		var obSelectedInput = BX.findChild(BX('selected_tasks_' + this.name), {tagName: 'input'});

		obSelectedInput.id = this.name + '_prop_task_selected_' + obInput.value;
		obSelectedInput.value = obInput.value;
	}
	else
	{
		...
		if (obInput.checked)
		{
			...
			if (!BX(this.name + "_task_selected_" + obInput.value))
			{
				...
				if (!BX(this.name + "_prop_task_selected_" + obInput.value)) {
					var selectedTasksBox = BX('selected_tasks_' + this.name);
					var obTaskInput = BX.create('INPUT');

					obTaskInput.id = this.name + '_prop_task_selected_' + obInput.value;
					obTaskInput.type = 'hidden';
					obTaskInput.name = this.inputName + '[n' + this.inputNum++ + ']';
					obTaskInput.value = obInput.value;

					selectedTasksBox.appendChild(obTaskInput);
				}
			}
		}
		else
		{
			...
			BX.remove(BX(this.name + "_prop_task_selected_" + obInput.value));
		}
	}
...
TasksTask.prototype.unselect = function(taskID, link)
{
	...
	BX.remove(BX(this.name + "_prop_task_selected_" + taskID));
};
