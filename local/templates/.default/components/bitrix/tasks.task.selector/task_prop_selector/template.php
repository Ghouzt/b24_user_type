...
$taskJs = $templateFolder . '/tasks.js';
...
<script type="text/javascript">
	...
	{
		O_<?=$name?> = new TasksTask("<?=$name?>", <?=($arParams["MULTIPLE"]? "true" : "false")?>, '', '<?= $arParams["~INPUT_NAME"]?>');
	}
	BX.ready(function()
	{
		BX.loadScript('<?=$taskJs?>', function() {
			...
			O_<?=$name?>.searchInput = BX("<?=CUtil::JSEscape($name)?>_task_input");
			<?else:?>
		...
		<tr>
			<td class="finder-box-left-column">
				...
				<div class="finder-box-tabs-content">
                    ...
                    <div id="selected_tasks_<?= $name?>">
                        <?if ($arParams['MULTIPLE']) :?>
                            <?foreach ($arParams['VALUE'] as $propId => $taskId) :?>
                                <?if ($taskId != 0) :?>
                                    <input id="<?= $name?>_prop_task_selected_<?= $taskId?>"
                                           type="hidden"
                                           name="<?= $arParams['~INPUT_NAME']?>[<?= $propId?>]"
                                           value="<?= $taskId?>"
                                    >
                                <?endif;?>
                            <?endforeach;?>
                        <?else:?>
                            <?if (!empty($arParams['VALUE'])) :?>
                                <input id="<?= $name?>_prop_task_selected_<?= $arParams['VALUE'][0]?>"
                                       type="hidden"
                                       name="<?= $arParams['~INPUT_NAME']?>"
                                       value="<?= $arParams['VALUE'][0]?>"
                                >
                            <?else:?>
                                <input id="<?= $name?>_prop_task_selected"
                                       type="hidden"
                                       name="<?= $arParams['~INPUT_NAME']?>"
                                       value="<?= $arParams['VALUE'][0]?>"
                                >
